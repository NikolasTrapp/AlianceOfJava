...
public class Main {
	...
	public static ArrayList<Personagem> personagens = new ArrayList<>(
			Arrays.asList(new Personagem(10, "Monge", 10, 12), new Personagem(10, "Ladino", 13, 12),
					new Personagem(10, "Guerreiro", 12, 12), new Personagem(10, "Barbaro", 15, 12),
					new Personagem(10, "Mago", 8, 12), new Personagem(10, "Clérigo", 7, 12),
					new Personagem(10, "Bardo", 9, 12)));
	...
	public static boolean iniciarRodada() {
		...
		while (inimigos.size() > 0 && personagem.getHp() > 0) {
			...
			for (Inimigo inimigo : inimigos) {
				...
				System.out.println(ANSI_RED + "O inimigo " + inimigo.getNome() + " desferiu a você uma quantidade de "
						+ dano + " de dano, lhe resta " + personagem.getHp() + " pontos de vida" + ANSI_RESET);
			}
			turno++;
	...
	public static void batalharComBoss(int n) {
		...
		while (chefao.getHp() > 0 && personagem.getHp() > 0) {
			...
			System.out.println("O inimigo " + chefao.getNome() + " desferiu a você uma quantidade de " + dano
	...
	public static void abrirBau() {
		...
		if (personagem.getEquipamento() == null) {
			...
			if (equipamento == personagem.getEquipamento()) {
				...
				System.out.println("Você deseja substituir o seu " + personagem.getEquipamento().getNome() + " por um "
						+ equipamento.getNome() + "? s - Sim, n - Não");
				char resposta = sc.next().charAt(0);
				if (resposta == 's' || resposta == 'S') {
					personagem.setEquipamento(equipamento);
					System.out.println("Você equipou um(a): " + equipamento.getNome());
